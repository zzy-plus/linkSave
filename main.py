...
def installSaves():
    ...
    extract_path = getUserDir() + r'\Documents\Euro Truck Simulator 2\profiles'  # 存档路径
...
def onSave(value, dic, saveName):
    # 默认只加载第一个存档
    game_sii_path = getUserDir() + '\\Documents\\Euro Truck Simulator 2\\profiles\\' + saveName + '\\save\\1\\game.sii'
    # 解密
    try:
...
if __name__ == '__main__':
    ...
    try:
        ...
        tk.Button(frame, text='传送', font=('宋体', 12), width=12, height=1, command=lambda : onSave(combox1.get(),dic,saveName)).place(x=250,y=115)
